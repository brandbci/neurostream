...
if brmiscfxns_ver.split(".") < brmiscfxns_ver_req.split("."):
    raise Exception("brpylib requires brMiscFxns " + brmiscfxns_ver_req +
...
def processheaders(curr_file, packet_fields):
    ...
    packet_format_str = "<" + "".join(
        [fmt for name, fmt, fun in packet_fields])
    ...
    packet_formatted = dict.fromkeys(
        [name for name, fmt, fun in packet_fields])
...
nev_header_dict = {
    "basic": [
        FieldDef("FileTypeID", "8s",
                 format_stripstring),  # 8 bytes   - 8 char array
        FieldDef("FileSpec", "2B",
                 format_filespec),  # 2 bytes   - 2 unsigned char
        ...
        FieldDef("TimeStampResolution", "I",
                 format_none),  # 4 bytes   - uint32
        FieldDef("SampleTimeResolution", "I",
                 format_none),  # 4 bytes   - uint32
        FieldDef("TimeOrigin", "8H",
                 format_timeorigin),  # 16 bytes  - 8 x uint16
        FieldDef("CreatingApplication", "32s",
                 format_stripstring),  # 32 bytes  - 32 char array
        FieldDef("Comment", "256s",
                 format_stripstring),  # 256 bytes - 256 char array
        FieldDef("NumExtendedHeaders", "I", format_none),
    ],  # 4 bytes   - uint32
    "ARRAYNME":
    FieldDef("ArrayName", "24s",
             format_stripstring),  # 24 bytes  - 24 char array
    "ECOMMENT":
    FieldDef("ExtraComment", "24s",
             format_stripstring),  # 24 bytes  - 24 char array
    "CCOMMENT":
    FieldDef("ContComment", "24s",
             format_stripstring),  # 24 bytes  - 24 char array
    "MAPFILE":
    FieldDef("MapFile", "24s",
             format_stripstring),  # 24 bytes  - 24 char array
    "NEUEVWAV": [
        FieldDef("ElectrodeID", "H", format_none),  # 2 bytes  - uint16
        FieldDef("PhysicalConnector", "B",
                 format_charstring),  # 1 byte   - 1 unsigned char
        FieldDef("ConnectorPin", "B",
                 format_charstring),  # 1 byte   - 1 unsigned char
        ...
        FieldDef("NumSortedUnits", "B",
                 format_charstring),  # 1 byte   - 1 unsigned char
        FieldDef("BytesPerWaveform", "B",
                 format_charstring),  # 1 byte   - 1 unsigned char
    ...
    "NEUEVLBL": [
        FieldDef("ElectrodeID", "H", format_none),  # 2 bytes  - uint16
        FieldDef("Label", "16s",
                 format_stripstring),  # 16 bytes - 16 char array
    ...
    "NSASEXEV": [
        ...
        FieldDef("DigitalInputConfig", "B",
                 format_digconfig),  # 1 byte   - 1 unsigned char
        FieldDef("AnalogCh1Config", "B",
                 format_anaconfig),  # 1 byte   - 1 unsigned char
        FieldDef("AnalogCh1DetectVal", "h", format_none),  # 2 bytes  - int16
        FieldDef("AnalogCh2Config", "B",
                 format_anaconfig),  # 1 byte   - 1 unsigned char
        FieldDef("AnalogCh2DetectVal", "h", format_none),  # 2 bytes  - int16
        FieldDef("AnalogCh3Config", "B",
                 format_anaconfig),  # 1 byte   - 1 unsigned char
        FieldDef("AnalogCh3DetectVal", "h", format_none),  # 2 bytes  - int16
        FieldDef("AnalogCh4Config", "B",
                 format_anaconfig),  # 1 byte   - 1 unsigned char
        FieldDef("AnalogCh4DetectVal", "h", format_none),  # 2 bytes  - int16
        FieldDef("AnalogCh5Config", "B",
                 format_anaconfig),  # 1 byte   - 1 unsigned char
    ...
    "VIDEOSYN": [
        ...
        FieldDef("VideoSource", "16s",
                 format_stripstring),  # 16 bytes - 16 char array
        FieldDef("FrameRate", "f", format_none),  # 4 bytes  - single float
        FieldDef("EmptyBytes", "2s", format_none),
    ],  # 2 bytes  - empty
    "TRACKOBJ": [
        FieldDef("TrackableType", "H",
                 format_trackobjtype),  # 2 bytes  - uint16
...
nsx_header_dict = {
    ...
    "basic": [
        ...
        FieldDef("TimeStampResolution", "I",
                 format_none),  # 4 bytes   - uint32
        FieldDef("TimeOrigin", "8H",
                 format_timeorigin),  # 16 bytes  - 8 uint16
        FieldDef("ChannelCount", "I", format_none),
    ],  # 4 bytes   - uint32
    "extended": [
        FieldDef("Type", "2s", format_stripstring),  # 2 bytes   - 2 char array
        FieldDef("ElectrodeID", "H", format_none),  # 2 bytes   - uint16
        FieldDef("ElectrodeLabel", "16s",
                 format_stripstring),  # 16 bytes  - 16 char array
...
def check_starttime(start_time_s):
    if not isinstance(start_time_s,
                      (int, float)) or (isinstance(start_time_s, (int, float))
                                        and start_time_s < START_TIME_DEF):
        print(
...
class NevFile:
    ...
    def __init__(self, datafile=""):
        ...
        self.basic_header = processheaders(self.datafile,
        ...
        for i in range(self.basic_header["NumExtendedHeaders"]):
            ...
            self.extended_headers[i].update(
                processheaders(
                    self.datafile,
                    nev_header_dict[self.extended_headers[i]["PacketID"]]))

            # Must set this for file spec 2.1 and 2.2
            if (header_string == "NEUEVWAV"
                    and float(self.basic_header["FileSpec"]) < 2.3):
                self.extended_headers[i][
                    "SpikeWidthSamples"] = WAVEFORM_SAMPLES_21
    ...
    def getdata(self, elec_ids="all", wave_read="read"):
        ...
        nPackets = int((lData - self.basic_header["BytesInHeader"]) /
        ...
        neuralPackets = [
            idx for idx, element in enumerate(PacketID)
        ...
        if len(neuralPackets) > 0:
            ...
            if type(elec_ids) is list:
                elecindices = [
                    idx for idx, element in enumerate(ChannelID[neuralPackets])
            ...
            if wave_read == "read":
                wfs = np.ndarray(
                    (
                        nPackets,
                        int((self.basic_header["BytesInDataPackets"] -
                             (tsBytes + 4)) / 2),
                    ),
                    "<h",
                    rawdata,
                ...
                output["spike_events"].update(
                    {"Waveforms": wfs[neuralPackets, :]})

        # digital events, i.e. digital inputs and serial inputs
        digiPackets = [
            idx for idx, element in enumerate(PacketID)
            if element == DIGITAL_PACKET_ID
        ]
        ...
        if len(commentPackets) > 0:
            ...
            ANSIPackets = [
                idx for idx, element in enumerate(charSet)
                if element == CHARSET_ANSI
            ]
            if len(ANSIPackets) > 0:
                charSetList[ANSIPackets] = "ANSI"
            UTFPackets = [
                idx for idx, element in enumerate(charSet)
                if element == CHARSET_UTF
            ]
            if len(UTFPackets) > 0:
            ...
            trueComments = np.setdiff1d(
                list(range(0,
                           len(commentPackets) - 1)), ROIPackets)
            ...
            subsetInds = list(
                set(list(range(0,
                               len(charSetList) - 1))) - set(ROIPackets))
            ...
            if len(ROIPackets) > 0:
                ...
                ROIfields = [
                    l.split(":") for l in ":".join(nmcommentsFinal).split(":")
                ]
        ...
        trackingPackets = [
            idx for idx, element in enumerate(PacketID)
            if element == TRACKING_PACKET_ID
        ]
        if len(trackingPackets) > 0:
            trackerObjs = [
                sub["VideoSource"] for sub in self.extended_headers
                if sub["PacketID"] == "TRACKOBJ"
            ]
            trackerIDs = [
                sub["TrackableID"] for sub in self.extended_headers
                if sub["PacketID"] == "TRACKOBJ"
            ]
            output["tracking"] = {
                "TrackerIDs":
                trackerIDs,
                "TrackerTypes": [
                    sub["TrackableType"] for sub in self.extended_headers
                    if sub["PacketID"] == "TRACKOBJ"
                ],
            }
            parentID = np.ndarray(
            ...
            for i in range(0, len(trackerObjs)):
                indices = [
                    idx for idx, element in enumerate(nodeID[trackingPackets])
                    if element == i
                ]
    ...
    def processroicomments(
        ...
        for i in range(len(comments["TimeStamps"])):
            if comments["CharSet"][i] == "NeuroMotive ROI":
                ...
                source_label = next(d["VideoSource"]
                ...
                if event == ENTER_EVENT:
                    roi_events["EnterTimeStamps"][idx].append(
                        comments["TimeStamp"][i])
                elif event == EXIT_EVENT:
                    roi_events["ExitTimeStamps"][idx].append(
                        comments["TimeStamp"][i])
...
class NsxFile:
    ...
    def __init__(self, datafile=""):
        ...
        self.basic_header["FileTypeID"] = bytes.decode(self.datafile.read(8),
                                                       "latin-1")

        self.basic_header[
            "SampleResolution"] = MAX_SAMP_PER_S  # Might come from the file some day.
    ...
    def getdata(self,
        ...
        if zeropad and self.basic_header["TimeStampResolution"] == 1e9:
            print(
                "zeropad does not work with ptp-timestamped data. Ignoring zeropad argument.\n"
            )
            zeropad = False
        if force_srate and self.basic_header["TimeStampResolution"] != 1e9:
            print(
                "force_srate only works with ptp timestamps in filespec >= 3.x. Ignoring force_srate argument.\n"
            )
            force_srate = False
        ...
        output["elec_ids"] = []
        output["data_headers"] = [
        ]  # List of dicts with fields Timestamp, NumDataPoints, data_time_s, BoH, BoD
        output["data"] = []  # List of ndarrays
        output["samp_per_s"] = self.basic_header[
            "SampleResolution"] / self.basic_header["Period"]

        # Pull some useful variables from the basic_header
        data_pt_size = self.basic_header["ChannelCount"] * DATA_BYTE_SIZE
        clk_per_samp = self.basic_header["Period"] * self.basic_header[
            "TimeStampResolution"] / self.basic_header["SampleResolution"]
        filespec_maj, filespec_min = tuple(
            [int(_) for _ in self.basic_header["FileSpec"].split(".")][:2])
        ...
        # Measure file size
        self.datafile.seek(self.basic_header["BytesInHeader"],
                           0)  # Reset to end of header / start of data
        ...
        if filespec_maj == 2 and filespec_min == 1:
            ...
            output["data_headers"].append({
                ...
                num_data_pts,
                "data_time_s":
                num_data_pts / output["samp_per_s"],
            })
            output["data"].append(
                np.memmap(self.datafile,
                          dtype=np.int16,
                          mode="r",
                          offset=eoh,
                          shape=(num_data_pts,
            ...
            ptp_dt = [("reserved", "uint8"), ("timestamps", "uint64"),
            ...
            if filespec_maj >= 3:
                ...
                struct_arr = np.memmap(self.datafile,
                                       dtype=ptp_dt,
                                       shape=npackets,
                                       offset=eoh,
                                       mode="r")
                self.datafile.seek(
                    eoh, 0
                )  # Reset to end-of-header in case memmap moved the pointer.
                samp_per_pkt = np.all(struct_arr["num_data_points"] ==
                                      1)  # Confirm 1 sample per packet

            if not samp_per_pkt:
                # Multiple samples per packet; 1 packet == 1 uninterrupted segment.
                while 0 < self.datafile.tell() < ospath.getsize(
                        self.datafile.name):
                    ...
                    timestamp = timestamp + (
                        clk_per_samp * np.arange(num_data_pts)
                    ).astype(np.int64 if ts_size == 8 else np.int32)
                ...
                seg_starts = np.hstack((0, 1 + np.argwhere(
                    np.diff(struct_arr["timestamps"]) >
                    seg_thresh_clk).flatten()))
                for seg_ix, seg_start_idx in enumerate(seg_starts):
                    seg_stop_idx = seg_starts[seg_ix + 1] if seg_ix < (
                        len(seg_starts) - 1) else (len(struct_arr) - 1)
                    offset = eoh + seg_start_idx * struct_arr.dtype.itemsize
                    num_data_pts = seg_stop_idx - seg_start_idx
                    seg_struct_arr = np.memmap(self.datafile,
                                               dtype=ptp_dt,
                                               shape=num_data_pts,
                                               offset=offset,
                                               mode="r")
        ...
        if start_time_s != START_TIME_DEF:
            ...
            b_keep = [
                _["Timestamp"][-1] >= test_start_ts
                for _ in output["data_headers"]
            ]
            output["data_headers"] = [
                _ for _, b in zip(output["data_headers"], b_keep) if b
            ]
        ...
        if data_time_s != DATA_TIME_DEF:
            ...
            data_time_ts = data_time_s * self.basic_header[
                "TimeStampResolution"]
            test_stop_ts = test_start_ts + data_time_ts
            b_keep = [
                _["Timestamp"][0] <= test_stop_ts
                for _ in output["data_headers"]
            ]
            output["data_headers"] = [
                _ for _, b in zip(output["data_headers"], b_keep) if b
        ...
        for ix, data_header in enumerate(output["data_headers"]):
            ...
            if ix == 0 and zeropad and data_header["Timestamp"][0] != 0:
                ...
                padsize = ceil(data_header["Timestamp"][0] /
                ...
                try:
                    ...
                    raise
                pad_ts = data_header["Timestamp"][0] - (
                    clk_per_samp * np.arange(1, padsize + 1)).astype(
                        np.int64)[::-1]
                data_header["Timestamp"] = np.hstack(
                    (pad_ts, data_header["Timestamp"]))
            ...
            if force_srate:
                ...
                seg_clks = data_header["Timestamp"][-1] - data_header[
                    "Timestamp"][0] + np.uint64(clk_per_samp)
                ...
                if n_insert > 0:
                    ...
                    insert_vals = (data[insert_inds] +
                                   data[insert_inds + 1]) / 2
                ...
                data_header["Timestamp"] = data_header["Timestamp"][0] + (
                    clk_per_samp * np.arange(data.shape[0])).astype(np.int64)

            if downsample > 1:
                data = data[::downsample]

            data_header["NumDataPoints"] = data.shape[0]
            data_header["data_time_s"] = data_header["NumDataPoints"] / output[
                "samp_per_s"]
    ...
    def savesubsetnsx(self,
        ...
        file_num = 1
        pausing = False
        datafile_datapt_size = self.basic_header[
            "ChannelCount"] * DATA_BYTE_SIZE
        ...
        if file_time_s:
            ...
            if self.basic_header["FileSpec"] == "2.1":
                ...
                file_size += (NSX_BASIC_HEADER_BYTES_22 +
                              NSX_EXT_HEADER_BYTES_22 * num_elecs + 5)
            print(
        ...
        if ospath.isfile(file_name + "_000" + file_ext):
            if "y" != input("\nFile '" + file_name.split("/")[-1] + "_xxx" +
                            file_ext + "' already exists, overwrite [y/n]: "):
        ...
        #          Channel ID, repack first 4 bytes, write to disk, then copy remaining 62 (66-4) bytes
            ...
            bytes_in_headers = (NSX_BASIC_HEADER_BYTES_22 +
                                NSX_EXT_HEADER_BYTES_22 * num_elecs)
            num_pts_header_pos = bytes_in_headers + 5
            subset_file.write(
                np.array(bytes_in_headers).astype(np.uint32).tobytes())
        ...
        while self.datafile.tell() != ospath.getsize(self.datafile.name):
            ...
            if self.basic_header["FileSpec"] == "2.1":
                packet_pts = (ospath.getsize(
                    self.datafile.name) - self.datafile.tell()) / (
            ...
            mm_length = (DATA_PAGING_SIZE //
                         datafile_datapt_size) * datafile_datapt_size
            num_loops = int(ceil(packet_pts * datafile_datapt_size /
                                 mm_length))
            packet_read_pts = 0
            ...
            for loop in range(num_loops):
                if loop == 0:
                    ...
                    if loop == (num_loops - 1):
                        num_pts = ((packet_pts * datafile_datapt_size) %
                                   mm_length) // datafile_datapt_size
                    else:
                        num_pts = mm_length // datafile_datapt_size
                ...
                if file_size and (file_size -
                                  subset_file.tell()) < DATA_PAGING_SIZE:

                    # number of points we can possibly write to current subset file
                    pts_can_add = (int((file_size - subset_file.tell()) //
                                       (num_elecs * DATA_BYTE_SIZE)) + 1)
                    ...
                    while pts_can_add < num_pts:
                        ...
                        print("Writing subset file: " +
                              ospath.split(subset_file.name)[1])

                        if self.basic_header["FileSpec"] == "2.1":
                            subset_file.write(
                                prior_file.read(32 + 4 * num_elecs))
                        else:
                            subset_file.write(
                                prior_file.read(bytes_in_headers))
                            subset_file.write(header_binary)
                            timestamp_new = (
                                ...
                                self.basic_header["Period"])
                            subset_file.write(
                                np.array(timestamp_new).astype(
                                    np.uint32).tobytes())
                            subset_file.write(
                                np.array(num_pts - pts_can_add).astype(
                                    np.uint32).tobytes())

                            prior_file.seek(num_pts_header_pos, 0)
                            prior_file.write(
                                np.array(prior_file_pkt_pts).astype(
                                    np.uint32).tobytes())
            ...
            datafile_pos += (self.basic_header["ChannelCount"] * packet_pts *
                             DATA_BYTE_SIZE)
            self.datafile.seek(datafile_pos, 0)

            # If using file_timing and there is pausing in data (multiple packets), let user know
            if (file_time_s and not pausing and
                (self.datafile.tell() != ospath.getsize(self.datafile.name))):
                pausing = True
                print(
